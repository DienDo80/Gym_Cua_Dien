...
# MÁY LỌC: TỰ ĐỘNG SỬA MỌI LỖI COPY/PASTE URL (Không cần lo dư dấu gạch chéo hay khoảng trắng)
SUPABASE_URL = SUPABASE_URL_RAW.strip().split('/rest/v1')[0].rstrip('/')
SUPABASE_KEY = SUPABASE_KEY_RAW.strip()
...
# KHỞI TẠO DỮ LIỆU ĐÁM MÂY LẦN ĐẦU
def init_data():
...
try:
    ...
    st.error(f"🚨 KẾT NỐI ĐÃ VÀO SUPABASE NHƯNG VẪN LỖI: {e}")
    st.stop()
...
# GIAO DIỆN
def main():
    st.set_page_config(page_title="Eco Gym V19 - Chốt Hạ", page_icon="⚡", layout="wide")
