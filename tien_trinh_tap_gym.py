...
# MÁY LỌC TỰ ĐỘNG GỌT SẠCH LỖI COPY/PASTE
clean_url = SUPABASE_URL_RAW.strip().replace('/rest/v1', '').rstrip('/')
clean_key = SUPABASE_KEY_RAW.strip()

st.set_page_config(page_title="Eco Gym V19 - Bất Tử", page_icon="🧬", layout="wide")

# ==========================================
# KÍNH HIỂN VI SOI LỖI
# ==========================================
if "postgresql://" in clean_url:
    st.error("🚨 LỖI CHÍ MẠNG: Đạo hữu đang dán chuỗi có chữ 'postgresql://' vào ô URL. Hãy quay lại Supabase -> Project Settings -> API để lấy đường link bắt đầu bằng 'https://' !!")
    st.stop()
if not clean_url.startswith("https://"):
    st.error(f"🚨 LỖI LINK SAI ĐỊNH DẠNG: URL của đạo hữu đang là `{clean_url}`. Nó bắt buộc phải bắt đầu bằng https://")
    st.stop()
...
try:
    ...
    st.error(f"🚨 LỖI KHÔNG THỂ KHỞI TẠO CLIENT: {e}")
    st.stop()

# ==========================================
# KHỞI TẠO DỮ LIỆU (100% CHỮ THƯỜNG)
# ==========================================
def init_data():
    try:
        ...
        st.error("🚨 LỖI ĐỌC/GHI VÀO SUPABASE (CHỤP MÀN HÌNH CÁI NÀY):")
        st.code(str(e))
        st.stop()

# Chạy trình tạo dữ liệu
init_data()

# ==========================================
# GIAO DIỆN APP CHÍNH THỨC
# ==========================================
...
def main():
    ...
    st.sidebar.title("🧬 ECO GYM V19")
    st.sidebar.success("✅ ĐÃ KẾT NỐI ĐÁM MÂY!")
    ...
    if menu == "📅 Tác Chiến Hàng Ngày":
        ...
        st.info(f"👉 **Tuần {current_week}:** {guide['sets']} Sets | {guide['reps']} Reps | RIR {guide['rir']}")
