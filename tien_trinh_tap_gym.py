...
def init_data():
    try:
        ...
        st.error("🚨 KÍNH CHIẾU YÊU ĐÃ BẮT ĐƯỢC LỖI THỰC SỰ TỪ SUPABASE:")
        st.code(str(e))
        st.warning("👉 Đạo hữu hãy copy đoạn chữ nằm trong khung màu xám/đen bên trên gửi cho bần đạo. Có nó, bần đạo sẽ chỉ ngay ra được đạo hữu đang mắc ở đâu (sai Key, chưa tạo Bảng, hay quên tắt RLS)!")
        st.stop() # Lệnh này chặn đứng App, không cho nổ lỗi đỏ lằng nhằng nữa
...
# GIAO DIỆN
def main():
    st.title("✅ NẾU ĐẠO HỮU THẤY DÒNG NÀY, APP ĐÃ PHI THĂNG THÀNH CÔNG!")
