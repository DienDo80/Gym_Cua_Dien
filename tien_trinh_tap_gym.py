...
def main():
    st.set_page_config(page_title="Eco Gym V34 - Veteran Flex", layout="wide")
    ...
    # ---------------------------------------------------------
    # MỤC 1: LỊCH TẬP (CÓ FLEX MODE CHO THÂN DƯỚI)
    # ---------------------------------------------------------
    if menu == "1. 📅 LỊCH TẬP (VETERAN)":
        ...
        for day in cal:
            ...
            with st.expander(f"{'📍' if idx == today_idx else '📅'} {name}: {focus}", expanded=(idx==today_idx)):
                ...
                # ĐỘC QUYỀN: FLEX MODE CHO BUỔI THÂN DƯỚI (Thứ 5: idx=3, Thứ 7: idx=5)
                if idx in [3, 5]:
                    st.markdown("---")
                    st.caption("🧬 **Flex Mode:** Do Thân dưới chỉ cần 1 buổi/tuần, bạn có thể bẻ lái mục tiêu buổi này để bơm bù các cơ yếu.")
                    cf1, cf2 = st.columns([3, 1])
                    flex_opt = cf1.selectbox("Chuyển đổi mục tiêu buổi tập:", 
                        ["Đùi / Mông / Bắp Chân (Mặc định)", "Chân (Lower)", "Tập Ngực (Bù Volume)", "Tập Lưng (Bù Volume)", "Tập Vai / Tay", "Linh Hoạt (Tự Chọn)"],
                ...
                if exs:
                    # Tự động Migration Nháp (Template)
                    ...
                    # Nếu focus có chứa upper/lower thì dùng cơ chế cũ, nếu flex thì dùng cơ chế phân cơ tự động
                    ...
                    if st.button(f"🏁 XÁC NHẬN HOÀN THÀNH BUỔI TẬP", key=f"finish_day_{idx}", use_container_width=True):
                        ...
                        if new_inserts:
                            ...
                            st.success("🎉 Đã CHỐT SỔ thành công! Volume đã được nạp.")
                            st.rerun()
                else:
    ...
    # ---------------------------------------------------------
        ...
        if curr_w > m_len:
            ...
            st.info("Khi qua chu kỳ mới, số set sẽ tự giảm 50% nhưng KHÔNG đẻ thêm dữ liệu rác.")
            ...
            if st.button("➡️ Hoàn thành Tuần -> Qua tuần tiếp theo (Tự động Reset Volume)"):
    ...
    # ---------------------------------------------------------
        ...
        if not logs.empty:
            ...
            if cw_logs.empty:
                st.info("Tuần này chưa có giọt mồ hôi nào rơi. Bảng đếm Volume đã được Reset sạch sẽ!")
            else:
            ...
            if st.button("🚨 Thực Thi Dọn Dẹp"):
                ...
                elif "3 THÁNG" in clean: cutoff = (datetime.date.today() - datetime.timedelta(days=90)).strftime("%Y-%m-%d")
                ...
                if ids_to_del:
                    for chunk in [ids_to_del[i:i + 100] for i in range(0, len(ids_to_del), 100)]:
                        supabase.table("workoutlog").delete().in_("id", chunk).execute()
                    st.success(f"Đã thanh tẩy {len(ids_to_del)} dòng dữ liệu cũ! Khung giáo án vẫn an toàn.")
                else:
    ...
    # ---------------------------------------------------------
        ...
        with st.form("body_info_manual"):
            ...
            c4, c5 = st.columns(2)
            goal = c4.selectbox("Phase:", ["Cutting", "Mini-cutting", "Bulking", "Recomposition"], index=0)
            target_cal = st.number_input("Target Calo Hàng Ngày (kcal)", value=int(u.get('daily_calories', 2200) or 2200), step=50)

            if st.form_submit_button("💾 Lưu Trữ"):
                supabase.table("userprofile").update({
                    "height": h, "weight": w, "body_fat": bf, "goal_type": goal, "daily_calories": target_cal
                }).eq("id", 1).execute()
                st.success("Đã ghi nhận!")
